    ...
    public class IniParserConfiguration : ICloneable
    {
        ...
        public string CommentString
        {
            ...
            set
            {
                ...
                foreach (var specialChar in StrSpecialRegexChars)
                    value = value.Replace(new string(specialChar, 1), @"\" + specialChar);

                CommentRegex = new Regex(string.Format(StrCommentRegex, value));
        ...
        protected const string StrCommentRegex = @"^{0}(.*)";
        protected const string StrSectionRegexStart = @"^(\s*?)";
        protected const string StrSectionRegexMiddle = @"{1}\s*[\p{L}\p{P}\p{M}_\""\'\{\}\#\+\;\*\%\(\)\=\?\&\$\,\:\/\.\-\w\d\s\\\~]+\s*";
        protected const string StrSectionRegexEnd = @"(\s*?)$";
        protected const string StrKeyRegex = @"^(\s*[_\.\d\w]*\s*)";
        protected const string StrValueRegex = @"([\s\d\w\W\.]*)$";
        protected const string StrSpecialRegexChars = @"[]\^$.|?*+()";
