    ...
    internal static class SDK
    {
        ...
        internal class OverlayManager : IDisposable
        {
            ...
            private void setupInstance(IntPtr parentWindowHandle, Direct2DRendererOptions options)
            {
                ...
                if (SDK.IsWindow(parentWindowHandle) == 0) throw new Exception("The parent window does not exist");

                SDK.RECT bounds = new SDK.RECT();
                SDK.GetRealWindowRect(parentWindowHandle, out bounds);
            ...
            private void windowServiceThread()
            {
                ...
                while (!exitThread)
                {
                    ...
                    IsParentWindowVisible = SDK.IsWindowVisible(ParentWindowHandle) != 0;
        ...
        #region Direct2DRender

        internal class Direct2DRenderer : IDisposable
        {
            #region private vars
            ...
            private void SetupInstance(Direct2DRendererOptions options)
            {
                ...
                if (SDK.IsWindow(options.Hwnd) == 0) throw new ArgumentException("The window does not exist (hwnd = 0x" + options.Hwnd.ToString("X") + ")");

                SDK.RECT bounds = new SDK.RECT();

                if (SDK.GetRealWindowRect(options.Hwnd, out bounds) == 0) throw new Exception("Failed to get the size of the given window (hwnd = 0x" + options.Hwnd.ToString("X") + ")");
        ...
        internal class Direct2DFontCreationOptions
        {
        ...
        internal class Direct2DScene : IDisposable
        {
        ...
        {
            private Random rng;
            private delegate IntPtr WndProc(IntPtr hWnd, SDK.WindowsMessage msg, IntPtr wParam, IntPtr lParam);
            ...
            private void windowThreadMethod(int x = 0, int y = 0, int width = 800, int height = 600)
            {
                ...
                while (true)
                {
                    ...
                    SDK.Message message = new SDK.Message();

                    if (SDK.PeekMessageW(ref message, WindowHandle, 0, 0, 1) != 0)
                    {
                        if (message.Msg == SDK.WindowsMessage.WM_QUIT) continue;

                        SDK.TranslateMessage(ref message);
                        SDK.DispatchMessage(ref message);
                    }
                }
            }
            ...
            private void setupInstance(int x = 0, int y = 0, int width = 800, int height = 600)
            {
                ...
                SDK.WNDCLASSEX wndClassEx = new SDK.WNDCLASSEX()
                {
                    cbSize = SDK.WNDCLASSEX.Size(),
                    style = 0,
                ...
                SDK.RegisterClassEx(ref wndClassEx);

                WindowHandle = SDK.CreateWindowEx(
                ...
                SDK.SetLayeredWindowAttributes(WindowHandle, 0, 255, /*0x1 |*/ 0x2);
                extendFrameIntoClientArea();
                SDK.UpdateWindow(WindowHandle);
            }

            private IntPtr windowProcedure(IntPtr hwnd, SDK.WindowsMessage msg, IntPtr wParam, IntPtr lParam)
            {
                switch (msg)
                {
                    case SDK.WindowsMessage.WM_DESTROY:
                        return (IntPtr)0;
                    case SDK.WindowsMessage.WM_ERASEBKGND:
                        SDK.SendMessage(WindowHandle, SDK.WindowsMessage.WM_PAINT, (IntPtr)0, (IntPtr)0);
                        break;
                    case SDK.WindowsMessage.WM_KEYDOWN:
                        return (IntPtr)0;
                    case SDK.WindowsMessage.WM_PAINT:
                        return (IntPtr)0;
                    case SDK.WindowsMessage.WM_DWMCOMPOSITIONCHANGED: // needed for windows 7 support
                        extendFrameIntoClientArea();
                        return (IntPtr)0;
                    default: break;
                }
                ...
                return SDK.DefWindowProc(hwnd, msg, wParam, lParam);
            }
            ...
            public void extendFrameIntoClientArea()
            {
                ...
                SDK.DwmExtendFrameIntoClientArea(WindowHandle, ref margin);
            }
            ...
            public void HideWindow()
            {
                ...
                SDK.ShowWindow(WindowHandle, 0);
                IsVisible = false;
            }

            public void MoveWindow(int x, int y)
            {
                SDK.MoveWindow(WindowHandle, x, y, Width, Height, 1);
                X = x;
                Y = y;
                extendFrameIntoClientArea();
            }
            ...
            protected virtual void Dispose(bool disposing)
            {
                if (!disposedValue)
                {
                    ...
                    SDK.DestroyWindow(WindowHandle);
                    SDK.UnregisterClass(randomClassName, IntPtr.Zero);
